...
def connect_mongodb(db_name,collection_name):
	global collection
	# connection
	conn = MongoClient("mongodb://admin:admin@192.168.1.52:27017") # 如果你只想連本機端的server你可以忽略，遠端的url填入: database://<user_name>:<user_password>@ds<xxxxxx>.mlab.com:<xxxxx>/<database_name>，請務必既的把腳括號的內容代換成自己的資料。
	db = conn[db_name]
